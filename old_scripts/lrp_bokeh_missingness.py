...
df = pd.DataFrame()
for labfile in glob.glob('../missingness/*.csv'):
    lfdf = pd.read_csv(labfile)
    df = pd.concat([df, lfdf])

df['Week_Submitted'] = pd.to_datetime(df['weekEnding'])
...
source = ColumnDataSource(
    data = dict(
        x = usedf['Week_Submitted'],
        y = usedf['n'], # included for the tooltips
...
allSource = ColumnDataSource(
    data = dict(
        Date_reported = df['Week_Submitted'],
        n = df['n'],
