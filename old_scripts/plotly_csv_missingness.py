...
# Import data:
df = pd.DataFrame()
for labfile in glob.glob('../processed_csvs/csv_missingness/*.csv'):
    lfdf = pd.read_csv(labfile)
    df = pd.concat([df, lfdf])

df['Date_reported'] = pd.to_datetime(df['weekEnding'])
...
fig.update_layout(
    title_text='Plotly Missingness Test',
    # hovermode = 'x',
    height=550,
    width=850,
    title_y=0.97,
...
fig.update_layout(
    updatemenus=[
        ...
        #             label='Patient Tel No. Missing',
        #             method='restyle'
        #         ),
        #         dict(
        #             args=[{'visible': [False, False, True, False]}],
        #             label='Symptom Onset Date Missing',
        #             method='restyle'
        #         ),
        #         dict(
...
fig.show()
fig.write_html("C:/Users/alexander.onopa/workspace/lrp/plotly_missingness.html")