...
def prep_hhie_data_list():
    ...
    labdfs = []
    for lab in all_labs:
        labdfs.append([lab, df.loc[df['Lab Name'] == lab]])
...
def compile_formatting(lablist):
    ...
    labdf_agg.to_csv('./data/processed/hhie_misformatting/' + lab2 + '_misformat.csv')
...
def check_hhie_parallel():
    ...
    pd.concat(data, axis=1).to_csv('./data/processed/hhie_all_misformat.csv')
