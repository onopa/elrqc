...
# for toxo analysis - not needed
# toxo_lookup = pd.read_csv('./lookups/Loinc_Toxo_Export_20231010233739.csv')
# covid_loinc_codes = list(toxo_lookup['LOINC_NUM'])
observed_loincs = []
...
def create_dict_elr(record):
    ...
    #data['Lab Name'] = try_get_value(record, 'MSH.3.1') # don't think I need this
    data['Lab Name'] = try_get_value(record, 'MSH.4.1')
    data['receiving_facility'] = try_get_value(record, 'MSH.6.1')
    ...
    data['patient_id'] = try_get_value(record, 'PID.3.1')
...
# def create_dict_elr(record):
#     data = {}
#     data['patientRace'] = get_race_codes(record, data)
#     return data

def parse_hl7_hhie():
    print('parsing HHIE HL7s...')
    ...
    record_count = 0
    covid_count = 0
    for filename in glob.iglob('./data/raw/hl7/hhie/*.txt'):
        #print(filename)
    ...
    df.to_csv('./data/processed/hl7/hhie_tabular_output.csv', index=False, columns=records[0].keys())
...
def move_hl7_elr():
    ...
    for f in glob.glob(aims_folder_input + '*.txt'):
        fname = os.path.basename(f)
        outpath = aims_folder_output + fname
        if not os.path.exists(outpath):
            shutil.copy2(f, outpath)
    print('moving HHIE HL7s...')
    for f in glob.glob(hhie_folder_input + '*.txt'):
        fname = os.path.basename(f)
...
if __name__ == '__main__':
    def process_hl7s():
        ...
        print(f"Moving files took {movetime:0.4f} seconds")
        print(f"Parallel parser took {parsetime:0.4f} seconds")
        # save loincs - used during toxo analysis
        #loinc_counts = pd.value_counts(pd.Series(observed_loincs))
        #loinc_counts.to_csv('C:/Users/alexander.onopa/workspace/toxo_loincs.csv')
    process_hl7s()

#    race_counts = pd.DataFrame(pd.Series(race_vals).value_counts())
#    print(race_counts)
    #race_counts.to_csv('./data/processed/hl7/race_counts.csv')
    # with open('./data/raw/hl7/2023-09-14-14-07-15-0494.txt') as hl7file:
    #     message = hl7file.read()
    #     record = hl7.parse(message.replace('\n', '\r'))
